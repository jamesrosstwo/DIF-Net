...
class DeformedImplicitField(nn.Module):
    def __init__(self,
        ...
        self.template_field = modules.SingleBVPNet(type=model_type, mode='mlp', hidden_features=hidden_num,
                                                   num_hidden_layers=3, in_features=3, out_features=1)
    ...
    def forward(self, model_input, gt, **kwargs):
        ...
        grad_sdf = torch.autograd.grad(sdf_final, [x], grad_outputs=torch.ones_like(sdf), create_graph=True)[
        ...
        losses = deform_implicit_loss(model_out, gt, loss_grad_deform=kwargs['loss_grad_deform'],
    ...
    def embedding(self, embed, model_input, gt):
        ...
        model_out = {'model_in': model_output['model_in'], 'model_out': sdf_final, 'latent_vec': embed,
                     'grad_sdf': grad_sdf}
